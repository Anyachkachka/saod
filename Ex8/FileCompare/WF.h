...
public:
	...
	//int Rows() const Ц возвращает количество строк матрицы ¬агнера-‘ишера;
	int Rows() const {
		return N + 1;
	}
	//int Columns() const Ц возвращает количество столбцов матрицы;
	int Columns() const {
		return M + 1;
	}
	//int Get(int i, int j) const Ц возвращает значение i - го, j - го элемента матрицы;
	int Get(int i, int j) const {
		return (pm[i + M * j]);
	}
	//void Set(int i, int j, int val) Ц задает значение i-го,j-го элемента матрицы;
	void Set(int i, int j, int val) {
		pm[i + M * j] = val;
	}
	//int Distance() const Ц возвращает рассто€ние между последовательност€ми, которыми проинициализирован объект.
	int Distance() const {
		return pm[(N + 1) * (M + 1) - 1];
	}
